...
#define TURN_180_MS       700  // TODO: calibrate - ms to spin right ~180 degrees at SPEED_ROTATE
...
// ================================================================
void spinRight180() {
  Serial.println(F("\n--- Spinning Right ~180 degrees (timed, Speed 60) ---"));

  unsigned long t_start = millis();
  while (millis() - t_start < TURN_180_MS) {
    if (IrReceiver.decode() && IrReceiver.decodedIRData.command == CMD_STAR) {
      Serial.println(F("E-STOP!")); restoreIR(); return;
    }
